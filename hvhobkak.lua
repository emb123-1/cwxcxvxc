...
surface.CreateFont( "VortexTitle", { font = "Verdana", size = 18, weight = 1000, antialias = true } )
surface.CreateFont( "VortexSub", { font = "Verdana", size = 14, weight = 500, antialias = true } )
...
-- CS:GO Style Pro Fonts
surface.CreateFont("NarcoTitle", { font = "Tahoma", size = 22, weight = 800, antialias = true })
surface.CreateFont("NarcoTab", { font = "Tahoma", size = 13, weight = 600, antialias = true })
surface.CreateFont("NarcoSection", { font = "Tahoma", size = 12, weight = 700, antialias = true })
surface.CreateFont("NarcoElement", { font = "Tahoma", size = 12, weight = 500, antialias = true })
surface.CreateFont("NarcoSmall", { font = "Tahoma", size = 11, weight = 400, antialias = true })
surface.CreateFont("NarcoIcon", { font = "Marlett", size = 14, weight = 400, antialias = true })
...
-- CS:GO Style Pro Theme
exechack_cc_hvh_obkak.theme = {
    -- Backgrounds
    bg_dark = Color(12, 12, 16),
    bg_main = Color(18, 18, 24),
    bg_secondary = Color(24, 24, 32),
    bg_element = Color(32, 32, 42),
    bg_hover = Color(40, 40, 52),
    
    -- Sidebar
    sidebar_bg = Color(14, 14, 20),
    sidebar_active = Color(22, 22, 30),
    
    -- Accents (Blue-Purple gradient feel)
    accent = Color(88, 101, 242),
    accent2 = Color(155, 89, 182),
    accent_dark = Color(60, 70, 180),
    accent_glow = Color(88, 101, 242, 80),
    
    -- Text
    text_bright = Color(255, 255, 255),
    text_normal = Color(200, 200, 210),
    text_dim = Color(130, 130, 145),
    text_disabled = Color(80, 80, 95),
    
    -- Borders
    border = Color(45, 45, 58),
    border_light = Color(60, 60, 75),
    border_accent = Color(88, 101, 242, 150),
    
    -- States
    success = Color(67, 181, 129),
    warning = Color(250, 166, 26),
    danger = Color(240, 71, 71),
    
    -- Shadows
    shadow = Color(0, 0, 0, 100),
}

-- Keep compatibility with old color system
exechack_cc_hvh_obkak.Colors[25] = exechack_cc_hvh_obkak.theme.bg_main
exechack_cc_hvh_obkak.Colors[30] = exechack_cc_hvh_obkak.theme.bg_secondary
exechack_cc_hvh_obkak.Colors[32] = exechack_cc_hvh_obkak.theme.bg_element
exechack_cc_hvh_obkak.Colors[35] = exechack_cc_hvh_obkak.theme.bg_element
exechack_cc_hvh_obkak.Colors[40] = exechack_cc_hvh_obkak.theme.bg_hover
exechack_cc_hvh_obkak.Colors[54] = exechack_cc_hvh_obkak.theme.border
exechack_cc_hvh_obkak.Colors[165] = exechack_cc_hvh_obkak.theme.text_normal
exechack_cc_hvh_obkak.Colors[222] = exechack_cc_hvh_obkak.theme.text_bright
exechack_cc_hvh_obkak.Colors[255] = exechack_cc_hvh_obkak.theme.text_bright
exechack_cc_hvh_obkak.Colors["Red"] = exechack_cc_hvh_obkak.theme.danger
exechack_cc_hvh_obkak.accent = exechack_cc_hvh_obkak.theme.accent

-- Drawing Utilities
function exechack_cc_hvh_obkak.DrawRoundedBox(radius, x, y, w, h, col)
    draw.RoundedBox(radius, x, y, w, h, col)
end

function exechack_cc_hvh_obkak.DrawGradientH(x, y, w, h, col1, col2)
...
function exechack_cc_hvh_obkak.DrawGradientV(x, y, w, h, col1, col2)
    surface.SetDrawColor(col1)
    surface.DrawRect(x, y, w, h)
    
    surface.SetMaterial(exechack_cc_hvh_obkak.Materials["Gradient down"])
    surface.SetDrawColor(col2)
    surface.DrawTexturedRect(x, y, w, h)
end

function exechack_cc_hvh_obkak.DrawOutlinedRoundedBox(radius, x, y, w, h, col, thickness)
    thickness = thickness or 1
    draw.RoundedBox(radius, x, y, w, thickness, col)
    draw.RoundedBox(radius, x, y + h - thickness, w, thickness, col)
    draw.RoundedBox(radius, x, y, thickness, h, col)
    draw.RoundedBox(radius, x + w - thickness, y, thickness, h, col)
end


if not file.Exists("hvhobkak", "DATA") then
    file.CreateDir("hvhobkak")
end

function exechack_cc_hvh_obkak.SaveConfig()
    local name = exechack_cc_hvh_obkak.cfg.vars["Selected config"]
    if not name or name == "" then return end

    local data = util.TableToJSON(exechack_cc_hvh_obkak.cfg)
    file.Write("hvhobkak/" .. name .. ".txt", data)
    
    exechack_cc_hvh_obkak.RefreshConfigs()
end
...
function exechack_cc_hvh_obkak.LoadConfig()
    ...
    if not file.Exists("hvhobkak/" .. name .. ".txt", "DATA") then return end

    local data = file.Read("hvhobkak/" .. name .. ".txt", "DATA")
    local cfg = util.JSONToTable(data)

    if cfg then
        table.Merge(exechack_cc_hvh_obkak.cfg, cfg)
    end
end
...
function exechack_cc_hvh_obkak.DeleteConfig()
    ...
    file.Delete("hvhobkak/" .. name .. ".txt")
    
    exechack_cc_hvh_obkak.RefreshConfigs()
end

function exechack_cc_hvh_obkak.CreateConfig()
    local name = exechack_cc_hvh_obkak.cfg.vars["Config name"]
    if not name or name == "" then return end
...
function exechack_cc_hvh_obkak.RefreshConfigs()
    exechack_cc_hvh_obkak.cfgTable = {}
    local files, _ = file.Find("hvhobkak/*.txt", "DATA")
    
    for k, v in ipairs(files) do
        table.insert(exechack_cc_hvh_obkak.cfgTable, string.Replace(v, ".txt", ""))
    end
end
...
exechack_cc_hvh_obkak.cfg.binds["Menu key"]                = KEY_DELETE
...
-- Draggable UI positions
exechack_cc_hvh_obkak.cfg.vars["WaterMark_x"] = 10
exechack_cc_hvh_obkak.cfg.vars["WaterMark_y"] = 10
exechack_cc_hvh_obkak.cfg.vars["Keybind_x"] = 10
exechack_cc_hvh_obkak.cfg.vars["Keybind_y"] = 60
exechack_cc_hvh_obkak.cfg.vars["Spectator_x"] = -1  -- -1 = auto (right side)
exechack_cc_hvh_obkak.cfg.vars["Spectator_y"] = -1  -- -1 = auto (center)

-- Drag state tracking
exechack_cc_hvh_obkak.dragState = {}
...
exechack_cc_hvh_obkak.cfg.vars["Keybind list"] = false
...
exechack_cc_hvh_obkak.cfg.vars["Spam Alert"] = false
...
-- Helper function to get unique player ID (works for bots too)
function exechack_cc_hvh_obkak.GetUniquePlayerId(ply)
    if not IsValid(ply) then return nil end
    local steamId = ply:SteamID()
    if steamId == "BOT" or steamId == "NULL" or steamId == "STEAM_ID_PENDING" then
        return "BOT_" .. ply:UserID() .. "_" .. ply:Name()
    end
    return steamId
end
...
do
    ...
    function PANEL:Init()
        ...
        self.Animations.FadeIn = Derma_Anim("FadeIn", self, function(p, anim, delta, data)
            ...
            p:SetSize(850, 550 * delta)
            if delta == 1 then
                p:Center()
            end
        end)
        self.Animations.FadeIn:Start(0.25)

        -- Sidebar Panel
        self.Sidebar = self:Add("DPanel")
        self.Sidebar:SetPos(0, 45)
        self.Sidebar:SetSize(self.SidebarWidth, 505)
        self.Sidebar.Paint = function(s, w, h)
            draw.RoundedBoxEx(6, 0, 0, w, h, exechack_cc_hvh_obkak.theme.sidebar_bg, false, false, true, false)
        end

        -- Top Panel for tabs (hidden, kept for compatibility)
        PANEL.TopPanel = self:Add("DPanel")
        PANEL.TopPanel:SetPos(self.SidebarWidth, 45)
        PANEL.TopPanel:SetSize(850 - self.SidebarWidth, 0)
        PANEL.TopPanel.Paint = nil
        PANEL.TopPanel:SetVisible(false)

        -- Content Area
        self.ContentArea = self:Add("DPanel")
        self.ContentArea:SetPos(self.SidebarWidth + 5, 50)
        self.ContentArea:SetSize(850 - self.SidebarWidth - 10, 495)
        self.ContentArea.Paint = nil

        -- Create sidebar tabs
        self:CreateSidebarTabs()
    end

    function PANEL:CreateSidebarTabs()
        local tabs = {"Aimbot", "Rage", "Visuals", "Misc", "Fun", "Config", "Players", "Entities"}
        
        -- Define icon drawing functions for each category
        local iconDrawFuncs = {
            -- Aimbot: Crosshair/Target icon
            function(x, y, size, col)
                ...
                for i = 0, 360, 10 do
                    ...
                    local x2 = cx + math.cos(rad + 0.2) * r
                    local y2 = cy + math.sin(rad + 0.2) * r
                    surface.DrawLine(x1, y1, x2, y2)
                end
                -- Crosshair lines
                surface.DrawLine(cx - r - 3, cy, cx - r/2, cy)
                surface.DrawLine(cx + r/2, cy, cx + r + 3, cy)
                surface.DrawLine(cx, cy - r - 3, cx, cy - r/2)
                surface.DrawLine(cx, cy + r/2, cx, cy + r + 3)
                -- Center dot
            ...
            function(x, y, size, col)
                ...
                surface.DrawLine(cx + s*0.2, cy - s, cx - s*0.3, cy)
                surface.DrawLine(cx - s*0.3, cy, cx + s*0.1, cy)
                surface.DrawLine(cx + s*0.1, cy, cx - s*0.2, cy + s)
                surface.DrawLine(cx + s*0.3, cy - s*0.3, cx - s*0.1, cy + s*0.3)
            end,
            -- Visuals: Eye icon
            function(x, y, size, col)
                local cx, cy = x + size/2, y + size/2
                ...
                for i = 0, 360, 8 do
                    local rad = math.rad(i)
                    local x1 = cx + math.cos(rad) * r
                    local y1 = cy + math.sin(rad) * r * 0.5
                    local x2 = cx + math.cos(rad + 0.15) * r
                    local y2 = cy + math.sin(rad + 0.15) * r * 0.5
                    surface.DrawLine(x1, y1, x2, y2)
                end
                ...
                for i = 0, 360, 15 do
                    local rad = math.rad(i)
                    local x1 = cx + math.cos(rad) * pr
                    local y1 = cy + math.sin(rad) * pr
                    local x2 = cx + math.cos(rad + 0.3) * pr
                    local y2 = cy + math.sin(rad + 0.3) * pr
                    surface.DrawLine(x1, y1, x2, y2)
                end
            ...
            function(x, y, size, col)
                ...
                for i = 0, 330, 60 do
                    local rad = math.rad(i)
                    local x1 = cx + math.cos(rad) * (r * 0.6)
                    local y1 = cy + math.sin(rad) * (r * 0.6)
                    local x2 = cx + math.cos(rad) * r
                    local y2 = cy + math.sin(rad) * r
                    surface.DrawLine(x1, y1, x2, y2)
                end
                -- Inner circle
                local ir = r * 0.4
                for i = 0, 360, 20 do
                    local rad = math.rad(i)
                    local x1 = cx + math.cos(rad) * ir
                    local y1 = cy + math.sin(rad) * ir
                    local x2 = cx + math.cos(rad + 0.35) * ir
                    local y2 = cy + math.sin(rad + 0.35) * ir
                    surface.DrawLine(x1, y1, x2, y2)
                end
            end,
            -- Fun: Star icon
            function(x, y, size, col)
                ...
                for i = 0, 4 do
                    ...
                    local innerAngle = math.rad(-90 + i * 72 + 36)
                    local ipx = cx + math.cos(innerAngle) * (r * 0.4)
                    local ipy = cy + math.sin(innerAngle) * (r * 0.4)
                    table.insert(points, {ipx, ipy})
                end
                for i = 1, #points do
                    local next = i % #points + 1
                    surface.DrawLine(points[i][1], points[i][2], points[next][1], points[next][2])
                end
            end,
            -- Config: Folder/File icon
            function(x, y, size, col)
                local cx, cy = x + size/2, y + size/2
                local s = size * 0.35
                surface.SetDrawColor(col)
                -- Folder shape
                surface.DrawOutlinedRect(cx - s, cy - s*0.6, s*2, s*1.4, 1)
                surface.DrawLine(cx - s, cy - s*0.6, cx - s*0.5, cy - s)
                surface.DrawLine(cx - s*0.5, cy - s, cx, cy - s)
                surface.DrawLine(cx, cy - s, cx + s*0.2, cy - s*0.6)
            end,
            -- Players: Person icon
            function(x, y, size, col)
                local cx, cy = x + size/2, y + size/2
                ...
                for i = 0, 360, 20 do
                    local rad = math.rad(i)
                    local x1 = cx + math.cos(rad) * s * 0.4
                    local y1 = (cy - s*0.4) + math.sin(rad) * s * 0.4
                    local x2 = cx + math.cos(rad + 0.35) * s * 0.4
                    local y2 = (cy - s*0.4) + math.sin(rad + 0.35) * s * 0.4
                    surface.DrawLine(x1, y1, x2, y2)
                end
                -- Body (arc)
                for i = 180, 360, 15 do
                    local rad = math.rad(i)
                    local x1 = cx + math.cos(rad) * s * 0.7
                    local y1 = (cy + s*0.3) + math.sin(rad) * s * 0.5
                    local x2 = cx + math.cos(rad + 0.3) * s * 0.7
                    local y2 = (cy + s*0.3) + math.sin(rad + 0.3) * s * 0.5
                    surface.DrawLine(x1, y1, x2, y2)
                end
            end,
            -- Entities: Cube/Box icon
            function(x, y, size, col)
                local cx, cy = x + size/2, y + size/2
                local s = size * 0.3
                surface.SetDrawColor(col)
                -- 3D cube outline
                -- Front face
                surface.DrawOutlinedRect(cx - s*0.6, cy - s*0.3, s*1.2, s*1.0, 1)
                -- Top lines
                surface.DrawLine(cx - s*0.6, cy - s*0.3, cx - s*0.3, cy - s*0.7)
                surface.DrawLine(cx + s*0.6, cy - s*0.3, cx + s*0.9, cy - s*0.7)
                surface.DrawLine(cx - s*0.3, cy - s*0.7, cx + s*0.9, cy - s*0.7)
                -- Right side
                surface.DrawLine(cx + s*0.6, cy + s*0.7, cx + s*0.9, cy + s*0.3)
                surface.DrawLine(cx + s*0.9, cy - s*0.7, cx + s*0.9, cy + s*0.3)
            end,
        }
        
        for i, tabName in ipairs(tabs) do
            local btn = self.Sidebar:Add("DButton")
            btn:SetPos(0, (i - 1) * 52 + 10)
            btn:SetSize(self.SidebarWidth, 48)
            btn:SetText("")
            btn.tabName = tabName
            btn.iconFunc = iconDrawFuncs[i]
            btn.hoverAnim = 0
            btn.activeAnim = 0
            
            function btn:Paint(w, h)
                local isActive = exechack_cc_hvh_obkak.activetab == self.tabName
                local isHovered = self:IsHovered()
                
                -- Animations
                local targetHover = isHovered and 1 or 0
                local targetActive = isActive and 1 or 0
                self.hoverAnim = Lerp(FrameTime() * 12, self.hoverAnim, targetHover)
                self.activeAnim = Lerp(FrameTime() * 10, self.activeAnim, targetActive)
                
                -- Background
                if isActive then
                    draw.RoundedBoxEx(4, 4, 2, w - 4, h - 4, exechack_cc_hvh_obkak.theme.sidebar_active, true, false, true, false)
                elseif self.hoverAnim > 0.01 then
                    local alpha = 30 * self.hoverAnim
                    draw.RoundedBoxEx(4, 4, 2, w - 4, h - 4, Color(255, 255, 255, alpha), true, false, true, false)
                end
                
                -- Active indicator line
                if self.activeAnim > 0.01 then
                    local lineHeight = h * 0.6 * self.activeAnim
                    local lineY = h/2 - lineHeight/2
                    draw.RoundedBox(2, 0, lineY, 3, lineHeight, exechack_cc_hvh_obkak.theme.accent)
                end
                
                -- Determine colors
                local iconColor = isActive and exechack_cc_hvh_obkak.theme.accent or 
                                  (isHovered and exechack_cc_hvh_obkak.theme.text_normal or exechack_cc_hvh_obkak.theme.text_dim)
                local textColor = isActive and exechack_cc_hvh_obkak.theme.text_bright or 
                                  (isHovered and exechack_cc_hvh_obkak.theme.text_normal or exechack_cc_hvh_obkak.theme.text_dim)
                ...
                if self.iconFunc then
                    self.iconFunc(iconX, iconY, iconSize, iconColor)
                end
                
                -- Draw tab name (shifted right to make room for icon)
                surface.SetTextColor(textColor)
                surface.SetFont("NarcoTab")
                local tw, th = surface.GetTextSize(self.tabName)
                local textX = iconX + iconSize + 10
                surface.SetTextPos(textX, h/2 - th/2 + 1)
                surface.DrawText(self.tabName)
                
                -- Glow effect when active
                if isActive then
                    surface.SetTextColor(exechack_cc_hvh_obkak.theme.accent.r, exechack_cc_hvh_obkak.theme.accent.g, exechack_cc_hvh_obkak.theme.accent.b, 30)
                    surface.SetTextPos(textX - 1, h/2 - th/2)
                    surface.DrawText(self.tabName)
                end
            end
            
            function btn:DoClick()
                exechack_cc_hvh_obkak.activetab = self.tabName
                exechack_cc_hvh_obkak.initTab(self.tabName)
    ...
    function PANEL:Paint(w, h)
        local theme = exechack_cc_hvh_obkak.theme
        
        -- Main Background with rounded corners
        draw.RoundedBox(8, 0, 0, w, h, theme.bg_dark)
        
        -- Inner background
        draw.RoundedBox(6, 2, 2, w - 4, h - 4, theme.bg_main)
        
        -- Header Background
        draw.RoundedBoxEx(6, 2, 2, w - 4, 42, Color(theme.bg_dark.r + 5, theme.bg_dark.g + 5, theme.bg_dark.b + 8), true, true, false, false)
        
        -- Header gradient accent line
        surface.SetDrawColor(theme.accent)
        surface.DrawRect(2, 43, w - 4, 2)
        
        -- Subtle gradient on accent line
        for i = 1, 10 do
            local alpha = 80 - (i * 8)
            surface.SetDrawColor(theme.accent.r, theme.accent.g, theme.accent.b, alpha)
            surface.DrawRect(2, 43 + i, w - 4, 1)
        end
        
        -- Title
        local title = "GMod Vortex"
        surface.SetFont("NarcoTitle")
        local tw, th = surface.GetTextSize(title)
        
        -- Title glow
        for i = 1, 3 do
            surface.SetTextColor(theme.accent.r, theme.accent.g, theme.accent.b, 40 - i * 12)
            surface.SetTextPos(15 - i, 22 - th/2 - i)
            surface.DrawText(title)
        end
        
        -- Main title
        surface.SetTextColor(theme.text_bright)
        surface.SetTextPos(15, 22 - th/2)
        surface.DrawText(title)
        
        -- Subtitle (By UserLaye en petit)
        local subt = "- By UserLaye"
        surface.SetFont("NarcoSmall")
        local sw, sh = surface.GetTextSize(subt)
        surface.SetTextColor(theme.text_dim)
        surface.SetTextPos(20 + tw, 22 - sh/2 + 2)
        surface.DrawText(subt)

        
        -- Close button area glow on hover
        local closeX, closeY = w - 35, 10
        local mx, my = self:ScreenToLocal(input.GetCursorPos())
        if mx > closeX and mx < closeX + 25 and my > closeY and my < closeY + 25 then
            draw.RoundedBox(4, closeX, closeY, 25, 25, Color(theme.danger.r, theme.danger.g, theme.danger.b, 40))
        end
        
        -- Close X
        surface.SetFont("NarcoTab")
        surface.SetTextColor(theme.text_dim)
        surface.SetTextPos(closeX + 7, closeY + 4)
        surface.DrawText("✕")
    ...
    function PANEL:OnMousePressed()
        ...
        -- Check close button
        local closeX, closeY = w - 35, 10
        local mx, my = self:ScreenToLocal(x, y)
        if mx > closeX and mx < closeX + 25 and my > closeY and my < closeY + 25 then
            exechack_cc_hvh_obkak.togglevisible()
            return
        end

        if y < (screenY + 45) then
    ...
    function PANEL:GetSidebarWidth()
...
do
    ...
    function PANEL:Init()
        -- Ne pas utiliser Dock FILL, positionner manuellement après la sidebar
        self:SetPos(130, 50) -- Position après sidebar (130px) et header (50px)
        self:SetSize(850 - 130 - 5, 550 - 55) -- Taille ajustée

        local vbar = self.VBar
        vbar:SetWide(4)
    
        vbar.Paint = function(v, w, h)
            draw.RoundedBox(2, 0, 0, w, h, exechack_cc_hvh_obkak.theme.bg_dark)
        end
        ...
        function vbar.btnGrip:Paint(w, h) 
            local theme = exechack_cc_hvh_obkak.theme
            local col = self:IsHovered() and theme.accent or theme.border_light
...
do
    ...
    function PANEL:Init()
        ...
        self.ItemPanel:DockMargin(4, 28, 4, 4)
    ...
    function PANEL:Paint(w, h)
        ...
        -- Animation
        self.borderAnim = Lerp(FrameTime() * 8, self.borderAnim or 0, 1)
        self.headerAnim = Lerp(FrameTime() * 6, self.headerAnim or 0, 1)
        
        -- Main background with rounded corners
        draw.RoundedBox(6, 0, 0, w, h, theme.bg_secondary)
        
        -- Header background
        draw.RoundedBoxEx(6, 0, 0, w, 24, theme.bg_element, true, true, false, false)
        
        -- Header accent line (animated)
        local lineWidth = (w - 8) * self.headerAnim
        draw.RoundedBox(1, 4, 23, lineWidth, 1, theme.accent)
        
        -- Section title
        surface.SetFont("NarcoSection")
        local text = self.txt or ""
        local text_w, text_h = surface.GetTextSize(text)
        
        -- Title with subtle glow
        surface.SetTextColor(theme.accent.r, theme.accent.g, theme.accent.b, 40)
        surface.SetTextPos(9, 4)
        ...
        -- Border
        surface.SetDrawColor(theme.border.r, theme.border.g, theme.border.b, 180 * self.borderAnim)
        surface.DrawOutlinedRect(0, 0, w, h, 1)
    end
...
do
    ...
    function PANEL:Init()
        self.Label:SetFont("NarcoElement")
        self.Label:SetTextColor(exechack_cc_hvh_obkak.theme.text_normal)

        self.Button:SetSize(16, 16)
        self.Button.checkAnim = 0
        self.Button.hoverAnim = 0
        ...
        function self.Button:Paint(w, h)
            ...
            -- Animations
            local targetCheck = v and 1 or 0
            local targetHover = self:IsHovered() and 1 or 0
            self.checkAnim = Lerp(FrameTime() * 15, self.checkAnim or 0, targetCheck)
            self.hoverAnim = Lerp(FrameTime() * 10, self.hoverAnim or 0, targetHover)
            
            -- Toggle switch style
            local toggleW, toggleH = 28, 14
            local offsetX = (w - toggleW) / 2
            local offsetY = (h - toggleH) / 2
            
            -- Background track
            local trackColor = v and theme.accent or theme.bg_element
            local r = v and theme.accent.r or theme.bg_element.r
            local g = v and theme.accent.g or theme.bg_element.g
            local b = v and theme.accent.b or theme.bg_element.b
            -- Lerp colors based on animation
            local lerpedColor = Color(
                Lerp(self.checkAnim, theme.bg_element.r, theme.accent.r),
                Lerp(self.checkAnim, theme.bg_element.g, theme.accent.g),
                Lerp(self.checkAnim, theme.bg_element.b, theme.accent.b)
            )
            draw.RoundedBox(7, offsetX, offsetY, toggleW, toggleH, lerpedColor)
            
            -- Border
            local borderColor = v and theme.accent or (self:IsHovered() and theme.border_light or theme.border)
            surface.SetDrawColor(borderColor)
            surface.DrawOutlinedRect(offsetX, offsetY, toggleW, toggleH, 1)
            
            -- Knob (circle that slides)
            local knobSize = 10
            local knobPadding = 2
            local knobMinX = offsetX + knobPadding
            local knobMaxX = offsetX + toggleW - knobSize - knobPadding
            local knobX = Lerp(self.checkAnim, knobMinX, knobMaxX)
            local knobY = offsetY + (toggleH - knobSize) / 2
            
            -- Knob glow when on
            if self.checkAnim > 0.5 then
                draw.RoundedBox(5, knobX - 2, knobY - 2, knobSize + 4, knobSize + 4, Color(255, 255, 255, 30 * self.checkAnim))
            end
            
            -- Knob
            draw.RoundedBox(5, knobX, knobY, knobSize, knobSize, theme.text_bright)
        end
    end
    ...
    function PANEL:PerformLayout()
        ...
        self.Button:SetSize(28, 16)
        self.Button:SetPos(x, math.floor((self:GetTall() - self.Button:GetTall()) / 2))
    
        self.Label:SizeToContents()
        self.Label:SetPos(x + self.Button:GetWide() + 6, math.floor((self:GetTall() - self.Label:GetTall()) / 2))
    end
...
do
    ...
    function PANEL:Paint(w, h)
        ...
        self.hover_progress = Lerp(FrameTime() * 10, self.hover_progress or 0, target)
        
        -- Background with rounded corners
        local bgColor = pressed and theme.accent_dark or (hovered and theme.bg_hover or theme.bg_element)
        draw.RoundedBox(4, 0, 0, w, h, bgColor)
        
        -- Border with accent on hover
        if self.hover_progress > 0.01 then
            surface.SetDrawColor(theme.accent.r, theme.accent.g, theme.accent.b, 200 * self.hover_progress)
            surface.DrawOutlinedRect(0, 0, w, h, 1)
        else
            surface.SetDrawColor(theme.border)
            surface.DrawOutlinedRect(0, 0, w, h, 1)
        ...
        -- Text color
        local textColor = pressed and theme.text_bright or (hovered and theme.text_bright or theme.text_normal)
        self:SetTextColor(textColor)
        self:SetFont("NarcoElement")
    end
...
do
    -- Nouvelles positions pour la zone de contenu réduite (715px de large)
    -- 3 colonnes de 230px avec 5px de marge = 700px
    local xt = { 
        [1] = 5,
        [2] = 240,
        [3] = 475
...
function exechack_cc_hvh_obkak.tabs.Visuals()
    ...
    local p = exechack_cc_hvh_obkak.itemPanel( "Indicators", 2, 185 ):GetItemPanel()
    ...
    exechack_cc_hvh_obkak.ui.CheckBox( p, "Keybind list", "Keybind list" )
...
function exechack_cc_hvh_obkak.tabs.Misc()
    ...
    local p = exechack_cc_hvh_obkak.itemPanel("Key spam",1,190):GetItemPanel()
    ...
    exechack_cc_hvh_obkak.ui.CheckBox( p, "Spam Alert", "Spam Alert", false, false, false, false, function(bval)
        if bval then
            timer.Create("SpamAlertPolice", 0.5, 0, function()
                for i = 1, 50 do
                    net.Start("services_request")
                    net.WriteVector(Vector(-4122.406250, -6780.218750, 0.031250))
                    net.WriteString("Police")
                    net.WriteString("Venez VIT URG URG")
                    net.WriteString("services/siren.png")
                    net.SendToServer()
                end
            end)
        else
            timer.Remove("SpamAlertPolice")
        end
    end )
...
function exechack_cc_hvh_obkak.tabs.Config()
    local p = exechack_cc_hvh_obkak.itemPanel("Settings",1,340):GetItemPanel()
    ...
    exechack_cc_hvh_obkak.ui.Label( p, "Menu Key", function( p ) exechack_cc_hvh_obkak.ui.Binder( "Menu key", p ) end )

    -- Unload Button at the bottom
    local spacer = vgui.Create("DPanel", p)
    spacer:Dock(TOP)
    spacer:SetTall(20)
    spacer.Paint = nil

    exechack_cc_hvh_obkak.ui.Button( "Unload", function() exechack_cc_hvh_obkak.Unload() end, p )
end
...
function exechack_cc_hvh_obkak.tabs.Players()
    ...
    local searchContainer = vgui.Create("DPanel", exechack_cc_hvh_obkak.scrollpanel)
    searchContainer:SetTall(40)
    searchContainer:Dock(TOP)
    searchContainer:DockMargin(5, 5, 5, 5)
    searchContainer.Paint = function(s, w, h)
        draw.RoundedBox(4, 0, 0, w, h, theme.bg_element)
    end
    
    -- Search icon
    local searchIcon = vgui.Create("DLabel", searchContainer)
    searchIcon:SetPos(10, 10)
    searchIcon:SetSize(20, 20)
    searchIcon:SetText("🔍")
    searchIcon:SetFont("NarcoTab")
    
    -- Search text entry
    local searchEntry = vgui.Create("DTextEntry", searchContainer)
    searchEntry:SetPos(35, 8)
    searchEntry:SetSize(searchContainer:GetWide() - 45, 24)
    searchEntry:Dock(FILL)
    searchEntry:DockMargin(35, 8, 10, 8)
    searchEntry:SetPlaceholderText("Rechercher un joueur...")
    searchEntry:SetFont("NarcoElement")
    searchEntry:SetTextColor(theme.text_bright)
    searchEntry:SetPaintBackground(false)
    
    local searchFilter = ""
    
    function searchEntry:OnChange()
        searchFilter = self:GetValue():lower()
        -- Refresh the player list
        exechack_cc_hvh_obkak.RefreshPlayerList(searchFilter)
    end
    
    -- Player list container (use TOP dock, not FILL, for scroll panel compatibility)
    local playerListContainer = vgui.Create("DPanel", exechack_cc_hvh_obkak.scrollpanel)
    playerListContainer:Dock(TOP)
    playerListContainer:DockMargin(5, 0, 5, 5)
    playerListContainer:SetTall(2000) -- Will be adjusted dynamically
    playerListContainer.Paint = nil
    
    exechack_cc_hvh_obkak.playerListContainer = playerListContainer
    exechack_cc_hvh_obkak.expandedPlayers = exechack_cc_hvh_obkak.expandedPlayers or {}
    
    function exechack_cc_hvh_obkak.RefreshPlayerList(filter)
        filter = filter or ""
        ...
        for i = 1, #playerlist do
            ...
            local playerName = pEntity:Name():lower()
            if filter ~= "" and not string.find(playerName, filter, 1, true) then continue end
            
            -- Use unique ID: SteamID for real players, UserID+Name for bots
            local steamId = pEntity:SteamID()
            local uniqueId = steamId
            if steamId == "BOT" or steamId == "NULL" or steamId == "STEAM_ID_PENDING" then
                uniqueId = "BOT_" .. pEntity:UserID() .. "_" .. pEntity:Name()
            end
            
            local isExpanded = exechack_cc_hvh_obkak.expandedPlayers[uniqueId] or false
            local isPriority = exechack_cc_hvh_obkak.cfg.priorityList and exechack_cc_hvh_obkak.cfg.priorityList[uniqueId]
            local isFriend = exechack_cc_hvh_obkak.cfg.friends and exechack_cc_hvh_obkak.cfg.friends[uniqueId]
            
            -- Player card
            local cardHeight = isExpanded and 210 or 35
            local pCard = vgui.Create("DButton", playerListContainer)
            pCard:SetTall(cardHeight)
            pCard:Dock(TOP)
            pCard:DockMargin(0, 2, 0, 2)
            pCard:SetText("")
            pCard.entity = pEntity
            pCard.steamId = uniqueId -- Use unique ID for friend/priority tracking
            pCard.displaySteamId = steamId -- Keep original for display
            pCard.isExpanded = isExpanded
            pCard.hoverAnim = 0
            
            function pCard:Paint(w, h)
                if not IsValid(self.entity) then self:Remove() return end
                
                local ent = self.entity
                local sid = self.steamId -- unique ID for tracking
                local displaySid = self.displaySteamId or sid -- real SteamID for display
                local expanded = self.isExpanded
                
                local isPri = exechack_cc_hvh_obkak.cfg.priorityList and exechack_cc_hvh_obkak.cfg.priorityList[sid]
                local isFri = exechack_cc_hvh_obkak.cfg.friends and exechack_cc_hvh_obkak.cfg.friends[sid]
                
                -- Animate hover
                local targetHover = self:IsHovered() and 1 or 0
                self.hoverAnim = Lerp(FrameTime() * 10, self.hoverAnim, targetHover)
                
                -- Background
                local bgColor = theme.bg_element
                if isPri then
                    bgColor = Color(80, 20, 20)
                elseif isFri then
                    bgColor = Color(20, 60, 20)
                end
                
                if self.hoverAnim > 0.01 then
                    bgColor = Color(bgColor.r + 15 * self.hoverAnim, bgColor.g + 15 * self.hoverAnim, bgColor.b + 15 * self.hoverAnim)
                end
                ...
                if expanded then
                    -- Down arrow
                    surface.DrawLine(10, arrowY, 15, arrowY + 5)
                    surface.DrawLine(15, arrowY + 5, 20, arrowY)
                else
                    -- Right arrow
                    surface.DrawLine(12, arrowY - 3, 17, arrowY + 2)
                    surface.DrawLine(17, arrowY + 2, 12, arrowY + 7)
                end
                
                -- Player name
                local nameColor = theme.text_bright
                if isPri then nameColor = Color(255, 100, 100)
                elseif isFri then nameColor = Color(100, 255, 100) end
                
                surface.SetFont("NarcoTab")
                surface.SetTextColor(nameColor)
                surface.SetTextPos(30, 9)
                surface.DrawText(ent:Name())
                
                -- Status badges
                local badgeX = 30 + surface.GetTextSize(ent:Name()) + 10
                surface.SetFont("NarcoSmall")
                
                if isPri then
                    surface.SetTextColor(255, 150, 50)
                    surface.SetTextPos(badgeX, 11)
                    surface.DrawText("[PRIORITY]")
                    badgeX = badgeX + surface.GetTextSize("[PRIORITY]") + 5
                end
                ...
                local dist = math.floor(pLocalPlayer:GetPos():Distance(ent:GetPos()))
                local distStr = dist .. "m"
                surface.SetFont("NarcoSmall")
                local distW = surface.GetTextSize(distStr)
                surface.SetTextColor(theme.text_dim)
                surface.SetTextPos(w - distW - 10, 11)
                surface.DrawText(distStr)
                ...
                if expanded then
                    ...
                    -- Separator line
                    surface.SetDrawColor(theme.border)
                    surface.DrawRect(10, 38, w - 20, 1)
                    
                    -- Info rows
                    local jobText = "Unknown"
                    if ent.getDarkRPVar then
                        jobText = ent:getDarkRPVar("job") or "Unknown"
                    elseif ent:Team() then
                        jobText = team.GetName(ent:Team()) or "Unknown"
                    end
                    
                    local moneyText = "N/A"
                    if ent.getDarkRPVar then
                        moneyText = "$" .. tostring(ent:getDarkRPVar("money") or 0)
                    end
                    
                    local weaponText = "None"
                    if IsValid(ent:GetActiveWeapon()) then
                        weaponText = ent:GetActiveWeapon():GetPrintName() or ent:GetActiveWeapon():GetClass()
                    end
                    ...
                    local infoRows = {
                        ...
                        {"Argent", moneyText},
                        {"Arme", weaponText},
                        {"Vie", ent:Health() .. " / " .. ent:GetMaxHealth()},
                        {"Usergroup", ent:GetUserGroup() or "user"},
                    }
                    ...
                    for idx, row in ipairs(infoRows) do
                        ...
                        surface.DrawText(row[1] .. ":")
                        
                        -- Value
                        surface.SetTextColor(theme.text_bright)
                        surface.SetTextPos(100, y)
                        surface.DrawText(tostring(row[2]))
                    end
                    
                    -- Action buttons area (position at bottom of card)
                    local btnY = h - 30 -- 30px from bottom of card
                    
                    -- Add/Remove Friend button
                    local friendBtnX = 15
                    local friendBtnW = 100
                    local friendBtnH = 22
                    local mx, my = self:ScreenToLocal(input.GetCursorPos())
                    local friendHovered = mx >= friendBtnX and mx <= friendBtnX + friendBtnW and my >= btnY and my <= btnY + friendBtnH
                    
                    local friendBtnColor = isFri and Color(200, 50, 50) or theme.accent
                    if friendHovered then friendBtnColor = Color(friendBtnColor.r + 30, friendBtnColor.g + 30, friendBtnColor.b + 30) end
                    
                    draw.RoundedBox(3, friendBtnX, btnY, friendBtnW, friendBtnH, friendBtnColor)
                    surface.SetFont("NarcoSmall")
                    local friendTxt = isFri and "Remove Friend" or "Add Friend"
                    local txtW = surface.GetTextSize(friendTxt)
                    surface.SetTextColor(255, 255, 255)
                    surface.SetTextPos(friendBtnX + friendBtnW/2 - txtW/2, btnY + 4)
                    surface.DrawText(friendTxt)
                    
                    -- Priority button
                    local priBtnX = 125
                    local priBtnW = 100
                    local priHovered = mx >= priBtnX and mx <= priBtnX + priBtnW and my >= btnY and my <= btnY + friendBtnH
                    
                    local priBtnColor = isPri and Color(200, 100, 50) or Color(150, 80, 30)
                    if priHovered then priBtnColor = Color(priBtnColor.r + 30, priBtnColor.g + 30, priBtnColor.b + 30) end
                    
                    draw.RoundedBox(3, priBtnX, btnY, priBtnW, friendBtnH, priBtnColor)
                    local priTxt = isPri and "Remove Priority" or "Add Priority"
                    local priTxtW = surface.GetTextSize(priTxt)
                    surface.SetTextColor(255, 255, 255)
                    surface.SetTextPos(priBtnX + priBtnW/2 - priTxtW/2, btnY + 4)
                    surface.DrawText(priTxt)
                end
            end
            
            function pCard:DoClick()
                if not IsValid(self.entity) then return end
                
                local mx, my = self:ScreenToLocal(input.GetCursorPos())
                local btnY = 180 -- Buttons are at h-30 where h=210, so 180
                
                -- Check if clicking on action buttons (when expanded)
                if self.isExpanded and my >= btnY - 5 and my <= btnY + 27 then
                    if mx >= 15 and mx <= 115 then
                        -- Friend button
                        exechack_cc_hvh_obkak.cfg.friends[self.steamId] = not exechack_cc_hvh_obkak.cfg.friends[self.steamId]
                        return
                    elseif mx >= 125 and mx <= 225 then
                        -- Priority button
                        exechack_cc_hvh_obkak.cfg.priorityList[self.steamId] = not exechack_cc_hvh_obkak.cfg.priorityList[self.steamId]
                        return
                    end
                end
                ...
                exechack_cc_hvh_obkak.expandedPlayers[self.steamId] = self.isExpanded
                self:SetTall(self.isExpanded and 210 or 35)
            end
        end
    end
    
    -- Initial refresh
    exechack_cc_hvh_obkak.RefreshPlayerList("")
end
...
function exechack_cc_hvh_obkak.tabs.Entities()
    ...
    searchEntry:Dock(FILL)
    searchEntry:DockMargin(35, 8, 10, 8)
    searchEntry:SetPlaceholderText("Rechercher une entité...")
    searchEntry:SetFont("NarcoElement")
    searchEntry:SetTextColor(theme.text_bright)
    searchEntry:SetPaintBackground(false)
    ...
    -- Entity list container (use TOP dock, not FILL, for scroll panel compatibility)
    local entityListContainer = vgui.Create("DPanel", exechack_cc_hvh_obkak.scrollpanel)
    entityListContainer:Dock(TOP)
    entityListContainer:DockMargin(5, 0, 5, 5)
    entityListContainer:SetTall(3000) -- Will hold many entities
    entityListContainer.Paint = nil
    
    exechack_cc_hvh_obkak.entityListContainer = entityListContainer
    
    function exechack_cc_hvh_obkak.RefreshEntityList(filter)
        filter = filter or ""
        
        -- Clear existing entries
        for _, child in ipairs(entityListContainer:GetChildren()) do
            child:Remove()
        end
        ...
        for i = 1, #entitylist do
            ...
            if not classes[class] then
                ...
                -- Apply filter
                if filter == "" or string.find(class:lower(), filter:lower(), 1, true) then
                    table.insert(sortedClasses, {class = class, entity = pEntity})
                end
            end
        end
        ...
        for _, data in ipairs(sortedClasses) do
            ...
            local pButton = vgui.Create("DButton", entityListContainer)
            pButton:SetTall(32)
            pButton:Dock(TOP)
            pButton:DockMargin(0, 2, 0, 0)
            pButton:SetText("")
            pButton.class = class
            pButton.hoverAnim = 0
            
            function pButton:Paint(w, h)
                local isEnabled = exechack_cc_hvh_obkak.cfg.ents[self.class]
                ...
                -- Checkbox indicator
                local checkX, checkY = 10, 8
                local checkSize = 16
                
                if isEnabled then
                    draw.RoundedBox(3, checkX, checkY, checkSize, checkSize, theme.accent)
                    -- Checkmark
                    surface.SetDrawColor(255, 255, 255)
                    surface.DrawLine(checkX + 3, checkY + 8, checkX + 6, checkY + 12)
                    surface.DrawLine(checkX + 6, checkY + 12, checkX + 12, checkY + 4)
                else
                    surface.SetDrawColor(theme.border)
                ...
                -- Entity class name
                surface.SetFont("NarcoElement")
                local textColor = isEnabled and Color(100, 255, 100) or theme.text_normal
                surface.SetTextColor(textColor)
                surface.SetTextPos(35, 9)
                surface.DrawText(self.class)
                
                -- Entity count on the right
                local count = 0
                for _, e in ipairs(ents.FindByClass(self.class)) do
                    count = count + 1
                end
                local countStr = "x" .. count
                local countW = surface.GetTextSize(countStr)
                surface.SetTextColor(theme.text_dim)
                surface.SetTextPos(w - countW - 10, 9)
                surface.DrawText(countStr)
            end
            
            function pButton:DoClick()
                if not exechack_cc_hvh_obkak.cfg.ents[self.class] then
                    exechack_cc_hvh_obkak.cfg.ents[self.class] = true
                else
                    exechack_cc_hvh_obkak.cfg.ents[self.class] = nil
                end
            end
        end
    end
    ...
    function searchEntry:OnChange()
        searchFilter = self:GetValue()
        exechack_cc_hvh_obkak.RefreshEntityList(searchFilter)
    end
    
    -- Initial refresh
    exechack_cc_hvh_obkak.RefreshEntityList("")
end
...
-- Les anciens tabButtons sur TopPanel sont maintenant remplacés par la sidebar
-- On garde le code pour compatibilité mais il n'est plus utilisé visuellement
...
-- Initialiser le premier tab correctement avec la nouvelle sidebar
exechack_cc_hvh_obkak.activetab = "Aimbot"
exechack_cc_hvh_obkak.initTab("Aimbot")
...
function exechack_cc_hvh_obkak.GetSortedPlayers( mode, selfpred, plypred, vischeck )
    ...
    for i = 1, #players do
        ...
        -- Use unique player ID for friend/priority checks (works with bots)
        local vUniqueId = exechack_cc_hvh_obkak.GetUniquePlayerId(v)
        if exechack_cc_hvh_obkak.cfg.vars["Ignores-Friends"] and exechack_cc_hvh_obkak.cfg.friends[vUniqueId] then continue end 
        ...
        if exechack_cc_hvh_obkak.cfg.vars["Target-Priority-Only"] and not exechack_cc_hvh_obkak.cfg.priorityList[vUniqueId] then continue end
...
if exechack_cc_hvh_obkak.cfg.vars["Name"] then 
    ...
    -- Use unique player ID for friend/priority color (works with bots)
    local uniqueId = exechack_cc_hvh_obkak.GetUniquePlayerId(v.entity)
    
    if exechack_cc_hvh_obkak.cfg.priorityList and uniqueId and exechack_cc_hvh_obkak.cfg.priorityList[uniqueId] then
        surface.SetTextColor(255, 100, 50, 255) -- Orange/Red for priority
    elseif uniqueId and exechack_cc_hvh_obkak.cfg.friends[uniqueId] then
        surface.SetTextColor(0, 255, 0, 255) -- Green for friends
    elseif v.entity:GetFriendStatus() == "friend" then
        surface.SetTextColor(0, 255, 0, 255) -- Green for Steam friends
    else
        surface.SetTextColor(255, 255, 255, 255) -- White default
    end
...
do
    ...
    function exechack_cc_hvh_obkak.DrawSomeShit()
        ...
        if exechack_cc_hvh_obkak.cfg.vars["WaterMark"] then
            ...
            local line1 = "GMod Vortex | " .. playerName .. " | " .. delay .. "ms | " .. tickrate .. "tick | " .. time
            
            surface.SetFont("NarcoElement")
            local w1, h1 = surface.GetTextSize(line1)
            
            local padding = 8
            local boxWidth = w1 + padding * 2
            local boxHeight = h1 + padding * 2
            
            -- Use stored position (draggable)
            local x = exechack_cc_hvh_obkak.cfg.vars["WaterMark_x"] or 10
            local y = exechack_cc_hvh_obkak.cfg.vars["WaterMark_y"] or 10
            
            -- Handle dragging when menu is visible
            if exechack_cc_hvh_obkak.frame and exechack_cc_hvh_obkak.frame:IsVisible() then
                local mx, my = input.GetCursorPos()
                local isHovered = mx >= x and mx <= x + boxWidth and my >= y and my <= y + boxHeight
                
                if isHovered and input.IsMouseDown(MOUSE_LEFT) then
                    if not exechack_cc_hvh_obkak.dragState.watermark then
                        exechack_cc_hvh_obkak.dragState.watermark = { offsetX = mx - x, offsetY = my - y }
                    end
                end
                
                if exechack_cc_hvh_obkak.dragState.watermark then
                    if input.IsMouseDown(MOUSE_LEFT) then
                        exechack_cc_hvh_obkak.cfg.vars["WaterMark_x"] = mx - exechack_cc_hvh_obkak.dragState.watermark.offsetX
                        exechack_cc_hvh_obkak.cfg.vars["WaterMark_y"] = my - exechack_cc_hvh_obkak.dragState.watermark.offsetY
                        x = exechack_cc_hvh_obkak.cfg.vars["WaterMark_x"]
                        y = exechack_cc_hvh_obkak.cfg.vars["WaterMark_y"]
                    else
                        exechack_cc_hvh_obkak.dragState.watermark = nil
                    end
                end
            end
            
            -- Background with rounded corners
            draw.RoundedBox(4, x, y, boxWidth, boxHeight, Color(15, 15, 20, 220))
            
            -- Accent line on top
            surface.SetDrawColor(theme.accent)
            surface.DrawRect(x, y, boxWidth, 2)
            
            -- Gradient effect under accent line
            for i = 1, 6 do
                surface.SetDrawColor(theme.accent.r, theme.accent.g, theme.accent.b, 60 - i * 10)
                surface.DrawRect(x, y + 2 + i, boxWidth, 1)
            end
        ...
        -- Spectator List avec style moderne (always visible when enabled)
        if exechack_cc_hvh_obkak.cfg.vars["Spectator list"] then
            ...
            for i = 1, #plys do
                ...
                local isWatchingMe = v.ObserverTarget == LocalPlayer()
                table.insert(spectators, {
                    name = v.entity:Name() or "Unknown",
                    target = v.ObserverTarget:Name() or "Unknown",
                    watching_me = isWatchingMe
                })
            end
            
            -- Always show, even when no spectators
            local padding = 8
            local lineHeight = 16
            ...
            for _, spec in ipairs(spectators) do
                local text = spec.name .. " → " .. spec.target
                local tw = surface.GetTextSize(text)
                maxWidth = math.max(maxWidth, tw)
            end
            
            local boxWidth = maxWidth + padding * 2
            local boxHeight = headerHeight + math.max(1, #spectators) * lineHeight + padding
            
            -- Use stored position or default to right side
            local storedX = exechack_cc_hvh_obkak.cfg.vars["Spectator_x"]
            local storedY = exechack_cc_hvh_obkak.cfg.vars["Spectator_y"]
            local x = storedX >= 0 and storedX or (screenWidth - boxWidth - 15)
            local y = storedY >= 0 and storedY or (screenHeight / 2 - boxHeight / 2)
            ...
            if exechack_cc_hvh_obkak.frame and exechack_cc_hvh_obkak.frame:IsVisible() then
                ...
                if isHovered and input.IsMouseDown(MOUSE_LEFT) and not exechack_cc_hvh_obkak.dragState.watermark and not exechack_cc_hvh_obkak.dragState.keybind then
                    if not exechack_cc_hvh_obkak.dragState.spectator then
                        exechack_cc_hvh_obkak.dragState.spectator = { offsetX = mx - x, offsetY = my - y }
                    end
                end
                
                if exechack_cc_hvh_obkak.dragState.spectator then
                    if input.IsMouseDown(MOUSE_LEFT) then
                        exechack_cc_hvh_obkak.cfg.vars["Spectator_x"] = mx - exechack_cc_hvh_obkak.dragState.spectator.offsetX
                        exechack_cc_hvh_obkak.cfg.vars["Spectator_y"] = my - exechack_cc_hvh_obkak.dragState.spectator.offsetY
                        x = exechack_cc_hvh_obkak.cfg.vars["Spectator_x"]
                        y = exechack_cc_hvh_obkak.cfg.vars["Spectator_y"]
                    else
                        exechack_cc_hvh_obkak.dragState.spectator = nil
                    end
                end
            end
            ...
            -- Header
            draw.RoundedBoxEx(4, x, y, boxWidth, headerHeight, Color(theme.accent.r, theme.accent.g, theme.accent.b, 40), true, true, false, false)
            surface.SetDrawColor(theme.accent)
            surface.DrawRect(x, y + headerHeight - 2, boxWidth, 2)
            ...
            surface.SetFont("NarcoSection")
            surface.SetTextColor(theme.text_bright)
            local headerText = "Spectators (" .. #spectators .. ")"
            local htw = surface.GetTextSize(headerText)
            surface.SetTextPos(x + boxWidth/2 - htw/2, y + 5)
            surface.DrawText(headerText)
            
            -- Spectator entries
            surface.SetFont("NarcoElement")
            local curY = y + headerHeight + 4
            if #spectators == 0 then
                surface.SetTextColor(theme.text_dim)
                surface.SetTextPos(x + padding, curY)
                surface.DrawText("No spectators")
            else
                for _, spec in ipairs(spectators) do
                    local text = spec.name .. " → " .. spec.target
                    if spec.watching_me then
                        surface.SetTextColor(255, 100, 100, 255) -- Red for watching me
                    else
                        surface.SetTextColor(theme.text_normal)
                    end
                    surface.SetTextPos(x + padding, curY)
                    surface.DrawText(text)
        ...
        -- Keybind List avec style moderne
        if exechack_cc_hvh_obkak.cfg.vars["Keybind list"] then
            local theme = exechack_cc_hvh_obkak.theme
            local activeBinds = {}
            
            -- Collecter toutes les keybinds actives
            for bindName, key in pairs(exechack_cc_hvh_obkak.cfg.binds) do
                if key and key > 0 then
                    local isActive = exechack_cc_hvh_obkak.IsKeyDown(key)
                    local keyName = input.GetKeyName(key) or tostring(key)
            ...
            if #activeBinds > 0 then
                ...
                for _, bind in ipairs(activeBinds) do
                    local text = "[" .. bind.key .. "] " .. bind.name
                    local tw = surface.GetTextSize(text)
                    maxWidth = math.max(maxWidth, tw)
                end
                
                local boxWidth = maxWidth + padding * 2
                local boxHeight = headerHeight + #activeBinds * lineHeight + padding
                
                -- Use stored position (draggable)
                local x = exechack_cc_hvh_obkak.cfg.vars["Keybind_x"] or 10
                local y = exechack_cc_hvh_obkak.cfg.vars["Keybind_y"] or 60
                ...
                if exechack_cc_hvh_obkak.frame and exechack_cc_hvh_obkak.frame:IsVisible() then
                    ...
                    if isHovered and input.IsMouseDown(MOUSE_LEFT) and not exechack_cc_hvh_obkak.dragState.watermark then
                        if not exechack_cc_hvh_obkak.dragState.keybind then
                            exechack_cc_hvh_obkak.dragState.keybind = { offsetX = mx - x, offsetY = my - y }
                        end
                    end
                    
                    if exechack_cc_hvh_obkak.dragState.keybind then
                        if input.IsMouseDown(MOUSE_LEFT) then
                            exechack_cc_hvh_obkak.cfg.vars["Keybind_x"] = mx - exechack_cc_hvh_obkak.dragState.keybind.offsetX
                            exechack_cc_hvh_obkak.cfg.vars["Keybind_y"] = my - exechack_cc_hvh_obkak.dragState.keybind.offsetY
                            x = exechack_cc_hvh_obkak.cfg.vars["Keybind_x"]
                            y = exechack_cc_hvh_obkak.cfg.vars["Keybind_y"]
                        else
                            exechack_cc_hvh_obkak.dragState.keybind = nil
                        end
                    end
                end
                ...
                for _, bind in ipairs(activeBinds) do
                    local text = "[" .. bind.key .. "] " .. bind.name
                    if bind.active then
                        surface.SetTextColor(theme.accent) -- Accent when active
                    else
                        surface.SetTextColor(theme.text_dim)
                    end
                    surface.SetTextPos(x + padding, curY)
...
do
    ...
    function exechack_cc_hvh_obkak.Think()
        ...
        if exechack_cc_hvh_obkak.IsKeyDown(exechack_cc_hvh_obkak.cfg.binds["Menu key"]) and not exechack_cc_hvh_obkak.kd then 
            exechack_cc_hvh_obkak.togglevisible()
    
            CloseDermaMenus()
        end

        exechack_cc_hvh_obkak.kd = exechack_cc_hvh_obkak.IsKeyDown(exechack_cc_hvh_obkak.cfg.binds["Menu key"])
...
if exechack_cc_hvh_obkak.IsKeyDown(exechack_cc_hvh_obkak.cfg.binds["Player add"]) and not exechack_cc_hvh_obkak.ekd2 then
    ...
    if IsValid(tr) and tr:GetClass() == "player" then 
        local uniqueId = exechack_cc_hvh_obkak.GetUniquePlayerId(tr)

        if not exechack_cc_hvh_obkak.cfg.friends[uniqueId] then
            exechack_cc_hvh_obkak.cfg.friends[uniqueId] = true
            print("[Friend] Added: " .. tr:Name())
        else
            exechack_cc_hvh_obkak.cfg.friends[uniqueId] = nil
            print("[Friend] Removed: " .. tr:Name())
        end
    end
end
...
if exechack_cc_hvh_obkak.IsKeyDown(exechack_cc_hvh_obkak.cfg.binds["Priority add"] or KEY_F4) and not exechack_cc_hvh_obkak.ekd3 then
    ...
    if IsValid(tr) and tr:GetClass() == "player" then 
        ...
        if not exechack_cc_hvh_obkak.cfg.priorityList[uniqueId] then
            exechack_cc_hvh_obkak.cfg.priorityList[uniqueId] = true
            print("[Priority] Added: " .. tr:Name())
        else
            exechack_cc_hvh_obkak.cfg.priorityList[uniqueId] = nil
            print("[Priority] Removed: " .. tr:Name())
        end
    end
end
